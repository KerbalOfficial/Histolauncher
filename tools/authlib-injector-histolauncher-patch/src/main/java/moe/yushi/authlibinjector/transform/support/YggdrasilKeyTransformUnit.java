...
import static moe.yushi.authlibinjector.internal.org.objectweb.asm.Opcodes.IRETURN;
...
public class YggdrasilKeyTransformUnit implements TransformUnit {
	...
	public static boolean verifyPropertySignature(Object propertyObj) {
		...
		byte[] data = propertyValue.getBytes(StandardCharsets.UTF_8);

		for (PublicKey customKey : PUBLIC_KEYS) {
			try {
				if (verifyWithKey(customKey, data, sig)) {
					return true;
				}
	...
	private static boolean verifyWithKey(PublicKey key, byte[] data, byte[] sig) throws GeneralSecurityException {
