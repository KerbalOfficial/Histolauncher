...
from core.modloaders._endpoints import LEGACY_FABRIC_META_API
...
def fetch_legacyfabric_game_versions() -> list[dict[str, Any]] | None:
    cached = _game_cache.get("game")
    if cached is not None:
        return cached
    try:
        data = _http_get_json(f"{LEGACY_FABRIC_META_API}/versions/game")
    except RuntimeError as exc:
        safe_print(f"[modloaders] Failed to fetch Legacy Fabric game versions: {exc}")
        return None
    if not isinstance(data, list):
        safe_print("[modloaders] Unexpected Legacy Fabric game versions response format")
        return None
    _game_cache.set("game", data)
    safe_print(f"[modloaders] Fetched {len(data)} Legacy Fabric game versions")
    return data


def supports_legacyfabric_mc_version(mc_version: str) -> bool:
    value = str(mc_version or "").strip()
    if not value:
        return False
    game_versions = fetch_legacyfabric_game_versions()
...
def fetch_legacyfabric_loaders(mc_version: str) -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        data = _http_get_json(f"{LEGACY_FABRIC_META_API}/versions/loader/{encoded_mc}")
    except RuntimeError as exc:
        safe_print(
            f"[modloaders] Failed to fetch Legacy Fabric loaders for {mc_version}: {exc}")
        return None
    if not isinstance(data, list):
        safe_print("[modloaders] Unexpected Legacy Fabric response format")
        return None
    _loaders_cache.set(cache_key, data)
    safe_print(
        f"[modloaders] Fetched {len(data)} Legacy Fabric loader versions for {mc_version}")
    return data


def get_legacyfabric_loaders_for_version(
    mc_version: str, stable_only: bool = False
) -> list[dict[str, Any]]:
    if not supports_legacyfabric_mc_version(mc_version):
        return []

    loaders = fetch_legacyfabric_loaders(mc_version)
    if not loaders:
        return []
...
def fetch_legacyfabric_loader_profile_libraries(
    ...
    try:
        ...
        profile = _http_get_json(
            f"{LEGACY_FABRIC_META_API}/versions/loader/{mc_enc}/{loader_enc}/profile/json"
        )
    except RuntimeError as exc:
        safe_print(
            "[modloaders] Failed to fetch Legacy Fabric profile libraries for "
    ...
    if deps:
        safe_print(
            f"[modloaders] Extracted {len(deps)} official Legacy Fabric libraries "
            f"from profile {mc_version}/{loader_version}"
        )
        return deps

    safe_print(
        f"[modloaders] Legacy Fabric profile {mc_version}/{loader_version} had no libraries")
    return None


def get_legacyfabric_loader_libraries(
    loader_version: str, mc_version: str
) -> list[tuple[str, str]]:
    safe_print(
        f"[modloaders] Fetching official Legacy Fabric libraries for {loader_version}...")
    profile_deps = fetch_legacyfabric_loader_profile_libraries(loader_version, mc_version)
    if profile_deps:
        return profile_deps

    safe_print(
        f"[modloaders] Using fallback dependencies for Legacy Fabric {loader_version}")
    return [
        (f"net.fabricmc:fabric-loader:{loader_version}", "https://maven.fabricmc.net/"),
        (
            f"net.legacyfabric:intermediary:{mc_version}",
